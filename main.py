"""
Universal Duplicate Finder (Android 16 Fixed)
- Hardcoded Manifest Permissions
...
def main(page: ft.Page):
    ...
    # ─── PERMISSION HANDLER (Android 16 Fix) ───
    ...
    def update_stats():
        stats_text.value = f"Selected: {len(app_state['selected_files'])}"
        page.update()
    ...
    def start_scan(e):
        ...
        def worker():
            ...
            try:
                ...
                status_text.value = f"Error reading folder: {str(ex)}"
            ...
            total = len(paths)
            if total == 0:
                status_text.value = "⚠ No files found!"
                scan_btn.disabled = False
                page.update()
                return
    ...
    def request_delete(e):
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Text(f"Delete {len(app_state['selected_files'])} files permanently?"),
            actions=[
    ...
    # ─── NAYA PERMISSION BUTTONS FUNCTION ───
    def ask_permissions(e):
        # 1. Broad Storage Permission (Files & Media)
        ph.request_permission(ft.PermissionType.MANAGE_EXTERNAL_STORAGE)
        # 2. Specific Photos & Videos (For Android 16 Granular media)
        ph.request_permission(ft.PermissionType.PHOTOS)
        ph.request_permission(ft.PermissionType.VIDEOS)
        page.snack_bar = ft.SnackBar(ft.Text("Storage popup requested. Allow 'All Files Access' if prompted!"))
        page.snack_bar.open = True
        page.update()

    perm_btn = ft.ElevatedButton(
        text="Grant Files & Media Permissions", 
        icon=ft.icons.SECURITY, 
        color=ft.colors.BLACK,
        bgcolor=ft.colors.AMBER, 
        on_click=ask_permissions
    )
    ...
    delete_btn = ft.ElevatedButton(content=ft.Row([ft.Icon(ft.icons.DELETE, color=ft.colors.WHITE), ft.Text("DELETE SELECTED", color=ft.colors.WHITE, weight="bold")], alignment=ft.MainAxisAlignment.CENTER), style=ft.ButtonStyle(bgcolor=ft.colors.RED_700, shape=ft.RoundedRectangleBorder(radius=8)), height=50, on_click=request_delete)
