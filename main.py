"""
Duplicate Photo Finder (Mobile APK Version)
- Pillow Library Removed (100% Android Compatible)
- Finds Exact Duplicate Copies instantly using MD5 Hash
"""
...
# ─── PURE PYTHON HASHING (NO PILLOW NEEDED) ───
def file_md5(path):
    h = hashlib.md5()
    try:
...
def main(page: ft.Page):
    page.title = "Duplicate Photo Finder"
    ...
    sub_folder_check = ft.Checkbox(label="Sub-folders bhi scan karein", value=True)
    ...
    stats_text = ft.Text("Selected: 0 | Space Saved: 0 MB", color=ft.colors.CYAN_400, weight="bold")
    ...
    def render_results():
        ...
        if not app_state["all_groups"]:
            results_col.controls.append(ft.Text("🎉 No exact duplicates found!", color=ft.colors.GREEN_400, size=18))
            page.update()
            return

        for grp in app_state["all_groups"]:
            results_col.controls.append(ft.Text(f"⚡ Exact Copy ({len(grp['files'])} photos)", color=ft.colors.RED_400, weight="bold"))
            
            row = ft.Row(scroll="auto", spacing=10)
            
            for i, path in enumerate(grp['files']):
                is_first = (i == 0)
                row.controls.append(create_photo_card(path, is_first))
    ...
    def create_photo_card(path, is_first):
        ...
        # Flet natively displays images, no Pillow required!
        img = ft.Image(src=path, width=100, height=100, fit=ft.ImageFit.COVER, border_radius=5)
        
        card = ft.Container(
            content=ft.Column([img, badge, ft.Text(f"{short}\n{sz_txt}", size=10, text_align="center")], alignment="center", spacing=2),
            border_radius=8,
            padding=5,
            ink=True,
        )
    ...
    def start_scan(e):
        ...
        if not folder or not os.path.isdir(folder):
            page.snack_bar = ft.SnackBar(ft.Text("⚠ Valid folder select karein!"))
        ...
        def worker():
            ...
            paths = []
            walk = os.walk(folder) if sub_folder_check.value else [(folder, [], os.listdir(folder))]
            
            status_text.value = "Scanning folders..."
            page.update()
            ...
            total = len(paths)
            if total == 0:
                status_text.value = "⚠ Is folder mein koi photo nahi mili!"
            ...
            for i, p in enumerate(paths):
                status_text.value = f"Analyzing: [{i+1}/{total}]"
    ...
    def confirm_delete(e):
        ...
        render_results()
        page.snack_bar = ft.SnackBar(ft.Text(f"✅ Deleted {done} photos! (Errors: {errors})", color=ft.colors.GREEN))
        page.snack_bar.open = True
        page.update()
    ...
    def request_delete(e):
        if not app_state["selected_files"]:
            page.snack_bar = ft.SnackBar(ft.Text("⚠ Koi photo select nahi ki!"))
            page.snack_bar.open = True
            page.update()
            return
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Text(f"Kya aap sach mein {len(app_state['selected_files'])} photos ko permanently delete karna chahte hain?"),
            actions=[
    ...
    def on_dialog_result(e: ft.FilePickerResultEvent):
        if e.path:
            ...
            status_text.value = f"Selected Folder: {os.path.basename(e.path)}"
            page.update() 
    ...
    scan_btn = ft.ElevatedButton(
        content=ft.Row([ft.Icon(ft.icons.PLAY_ARROW, color=ft.colors.WHITE), ft.Text("FIND EXACT COPIES", color=ft.colors.WHITE, weight="bold")], alignment=ft.MainAxisAlignment.CENTER),
        style=ft.ButtonStyle(bgcolor=ft.colors.BLUE_700, shape=ft.RoundedRectangleBorder(radius=8)),
        height=50, on_click=start_scan
    )
    
    delete_btn = ft.ElevatedButton(
        content=ft.Row([ft.Icon(ft.icons.DELETE, color=ft.colors.WHITE), ft.Text("DELETE SELECTED", color=ft.colors.WHITE, weight="bold")], alignment=ft.MainAxisAlignment.CENTER),
        style=ft.ButtonStyle(bgcolor=ft.colors.RED_700, shape=ft.RoundedRectangleBorder(radius=8)),
        height=50, on_click=request_delete, disabled=True
    )

    button_layout = ft.Column([scan_btn, delete_btn], spacing=10, horizontal_alignment=ft.CrossAxisAlignment.STRETCH)

    page.add(
        ft.Row([ft.Icon(ft.icons.IMAGE_SEARCH, size=30, color=ft.colors.BLUE_400), ft.Text("Duplicate Finder Flet", size=22, weight="bold")], alignment=ft.MainAxisAlignment.CENTER),
